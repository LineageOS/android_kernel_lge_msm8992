	...
	struct mmc_card *oldcard)
{
	...
	if (!oldcard) {
		...
		if(card->cid.manfid == 0x90) { //SK Hynix request to change below values due to firmware bug
			card->ext_csd.part_time = 10 * 10; // change PARTITION_SWITCH_TIME[199] value to 0xA(100ms)
			card->ext_csd.generic_cmd6_time = 10 * 10; //chagne GENERIC_CMD6_TIME[248] value to 0xA(100ms)
		}
#endif
