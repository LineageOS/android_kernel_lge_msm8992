		...
		if (ctrl_pdata->ndx) {
			...
			case PROXY_NEAR:
				if (ctrl_pdata->panel_data.panel_info.lge_pan_info.panel_type
					...
					ctrl_pdata->panel_data.panel_info.lge_pan_info.panel_type
...
int lgd_lg4945_qhd_command_pre_mdss_dsi_panel_power_ctrl(struct mdss_panel_data *pdata, int enable)
{
	...
	if(enable)
	{
		...
		if (!ctrl_pdata->ndx) {
			...
			if (ret == 0 && !ctrl_pdata->lge_pan_data->always_on_power_ctrl) {
#else
			if (ret == 0 ) {
#endif
				mdss_dsi_lcd_reset(pdata, 0);
...
int lgd_qhd_command_dsi_panel_set_voltage(struct device_node *supply_node,
		struct dss_vreg *vreg_config, char *cmd_key, int num_of_rev)
{
	...
	sprintf(common_lgd_sic4945_propname, "%s%d", "qcom,supply-min-voltage-rev", lgd_sic4945_rev);
	rc = of_property_read_u32(supply_node,
		common_lgd_sic4945_propname, &tmp);
	if (rc) {
		pr_err("%s: error reading min volt. rc=%d\n",
			__func__, rc);
		goto error;
	}
	vreg_config->min_voltage = tmp;

	/* vreg-max-voltage */
	memset(common_lgd_sic4945_propname, 0, sizeof(char) * MAX_LEN_OF_PROPNAME);
	sprintf(common_lgd_sic4945_propname, "%s%d", "qcom,supply-max-voltage-rev", lgd_sic4945_rev);
	rc = of_property_read_u32(supply_node,
		common_lgd_sic4945_propname, &tmp);
	if (rc) {
	...
	vreg_config->max_voltage = tmp;

	pr_info("%s: %s voltage is modified. min=%d max=%d DDIC rev=%d",
		__func__, vreg_config->vreg_name, vreg_config->min_voltage, vreg_config->min_voltage,
		lgd_sic4945_rev);
...
EXPORT_SYMBOL(lgd_qhd_command_dsi_panel_set_voltage);
#endif
